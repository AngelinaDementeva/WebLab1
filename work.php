...
elseif ($r<1 || $r>4 || !is_numeric($r)) $fail .= "Некорректное значение R\n";
if ($fail != "") die($fail);

if ($x>=0 && $x<=$r/2 && $y>=0 && $y<=$r) $check=true;
elseif ($x>=0 && $y<=0 && $x<=sqrt($r*$r/4 - $y*$y)) $check=true;
elseif ($x<=0 && $y<=0 && $x>=-($y+$r)/2) $check=true;
$finish = microtime(true);
$time = number_format($finish-$start,6);

    $dt = new DateTime("now", new DateTimeZone('Europe/Moscow'));
$tags = array(
	'center' => 'font-family:  Arial, serif;',
);
